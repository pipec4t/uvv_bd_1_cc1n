...
SET SEARCH_PATH TO hr, "$user", public;
  ...
  /*
  Agora iremos criar as tabelas do Schema hr
  */
...
COMMENT ON TABLE cargos IS 'Tabela cargos onde é armazenado o número, cargo, salário mínimo e o salário máximo dos cargos da empresa.';
COMMENT ON COLUMN cargos.id_cargo IS 'Liga cada cargo a um número. 
É a Primary Key da tabela cargos.';
COMMENT ON COLUMN cargos.cargo IS 'É o nome dos cargos disponíveis.';
COMMENT ON COLUMN cargos.salario_minimo IS 'É o menor valor aceitável para determinado cargo.';
COMMENT ON COLUMN cargos.salario_maximo IS 'É o maior valor aceitável para determinado cargo.';
...
COMMENT ON TABLE regioes IS 'Tabela regiões onde é armazenado o número e o nome de cada região.';
COMMENT ON COLUMN regioes.id_regiao IS 'Liga cada região a um número. 
É a Primary Key da tabela regiões.';
COMMENT ON COLUMN regioes.nome IS 'É o nome de cada região. 
É uma Alternate Key da tabela regiões.';
...
COMMENT ON TABLE paises IS 'Tabela países onde é armazenado o número e o nome de cada país, assim como a região onde o país se encontra.';
COMMENT ON COLUMN paises.id_pais IS 'Liga cada país a um número. 
É a Primary Key da tabela países.';
COMMENT ON COLUMN paises.nome IS 'É o nome de cada país. 
É uma Alternate Key da tabela países.';
COMMENT ON COLUMN paises.id_regiao IS 'Identifica as regiões dos países. 
Foreign Key conectando a tabela regiões.';
...
COMMENT ON TABLE localizacoes IS 'Tabela localizações onde é armazenado o número, endereço, CEP, cidade e UF das localizações onde se encontra a empresa. Também armazena o país em que a localização se encontra.';
COMMENT ON COLUMN localizacoes.id_localizacao IS 'Liga cada localização a um número. 
É a Primary Key da tabela localizações.';
COMMENT ON COLUMN localizacoes.endereco IS 'É o endereço de cada local da empresa.';
COMMENT ON COLUMN localizacoes.cep IS 'É o CEP de cada local da empresa.';
COMMENT ON COLUMN localizacoes.cidade IS 'É a cidade de cada local da empresa.';
COMMENT ON COLUMN localizacoes.uf IS 'É o Estado de cada local da empresa.
Pode ser abreviado ou por extenso.';
COMMENT ON COLUMN localizacoes.id_pais IS 'Identifica o país de cada local da empresa.
Foreign Key conectando a tabela países.';
...
COMMENT ON TABLE departamentos IS 'Tabela departamentos onde é armazenado o número e o nome dos departamentos. 
Também é armazenado a localização e o gerente de cada departamento.';
COMMENT ON COLUMN departamentos.id_departamento IS 'Liga cada departamento a um número. 
É a Primary Key da tabela departamentos.';
COMMENT ON COLUMN departamentos.nome IS 'É o nome de cada departamento.';
COMMENT ON COLUMN departamentos.id_localizacao IS 'Identifica a localização dos departamentos. 
Foreign Key conectando a tabela localizações.';
COMMENT ON COLUMN departamentos.id_gerente IS 'Identifica o empregado gerente de cada departamento. 
Foreign Key conectando a tabela empregados.';
...
COMMENT ON TABLE empregados IS 'Tabela Empregados onde é armazenado os dados básicos dos empregados, como o número, nome, email, telefone, data de contratação, cargo, salário, comissão, departamentos, e o seu supervisor.';
COMMENT ON COLUMN empregados.id_empregado IS 'Liga cada empregado a um número. 
É a Primary Key da tabela empregados.';
COMMENT ON COLUMN empregados.nome IS 'É o nome completo do empregado.';
COMMENT ON COLUMN empregados.email IS 'É a parte inicial do email do empregado (antes do @).
Também é uma Alternate Key da tabela empregados.';
COMMENT ON COLUMN empregados.telefone IS 'É o número de telefone de cada empregado.';
COMMENT ON COLUMN empregados.data_contratacao IS 'É a data em que o empregado foi contratado para o emprego.';
COMMENT ON COLUMN empregados.id_cargo IS 'Identifica os cargos atuais dos empregados. 
Foreign Key conectando a tabela cargos.';
COMMENT ON COLUMN empregados.salario IS 'É o valor mensal que o empregado recebe.';
COMMENT ON COLUMN empregados.comissao IS 'É a porcentagem de comissão que um empregado receberá.
Somente empregados do departamento de vendas podem receber comissões.';
COMMENT ON COLUMN empregados.id_departamento IS 'Identifica o departamento atual dos empregados. 
Foreign Key conectando a tabela departamentos.';
COMMENT ON COLUMN empregados.id_supervisor IS 'Identifica o supervisor de um empregado (pode ou não ser o manager_id do departamento do empregado). 
Foreign Key conectando a própria tabela empregados com ela mesma.';
...
COMMENT ON TABLE historico_cargos IS 'Tabela cargos onde é armazenado o histórico dos cargos de cada funcionário, contendo o empregado, a data inicial, data final, o cargo e o departamento. 
Caso um empregado mude de departamento dentro de um cargo, ou mudar de cargo dentro de um departamento, outras linhas novas devem ser inseridas na tabela com a informação antiga.';
COMMENT ON COLUMN historico_cargos.id_empregado IS 'Liga cada empregado a um número. 
Uma das Primary Keys compostas (id_empregado e data_inicial) da tabela historico_cargos.
Também é uma Foreign Key conectando a tabela empregados.';
COMMENT ON COLUMN historico_cargos.data_inicial IS 'Indica a data inicial de um funcionário em um cargo.
É uma das Primary Keys compostas (id_empregado e data_inicial) da tabela historico_cargos.
Deve ser menor que a data final.';
COMMENT ON COLUMN historico_cargos.data_final IS 'Indica a data final de um funcionário em um cargo.
Deve ser maior que a data inicial.';
COMMENT ON COLUMN historico_cargos.id_cargo IS 'Identifica o cargo antigo do empregado. 
Foreign Key conectando a tabela cargos.';
COMMENT ON COLUMN historico_cargos.id_departamento IS 'Identifica o departamento antigo do empregado. 
Foreign Key conectando a tabela departamentos.';
...
INSERT INTO departamentos (id_departamento, nome, id_localizacao, id_gerente)
VALUES (30, 'Purchasing', 1700, 114);
INSERT INTO departamentos (id_departamento, nome, id_localizacao, id_gerente)
VALUES (40, 'Human Resources', 2400, 203);
INSERT INTO departamentos (id_departamento, nome, id_localizacao, id_gerente)
VALUES (50, 'Shipping', 1500, 121);
INSERT INTO departamentos (id_departamento, nome, id_localizacao, id_gerente)
VALUES (60, 'IT', 1400, 103);
INSERT INTO departamentos (id_departamento, nome, id_localizacao, id_gerente)
VALUES (70, 'Public Relations', 2700, 204);
INSERT INTO departamentos (id_departamento, nome, id_localizacao, id_gerente)
VALUES (80, 'Sales', 2500, 145);
INSERT INTO departamentos (id_departamento, nome, id_localizacao, id_gerente)
VALUES (90, 'Executive', 1700, 100);
INSERT INTO departamentos (id_departamento, nome, id_localizacao, id_gerente)
VALUES (100, 'Finance', 1700, 108);
INSERT INTO departamentos (id_departamento, nome, id_localizacao, id_gerente)
VALUES (110, 'Accounting', 1700, 205);
INSERT INTO departamentos (id_departamento, nome, id_localizacao, id_gerente)
...
/* 
Agora todo o Projeto Lógico HR está criado no PostgreSQL!
